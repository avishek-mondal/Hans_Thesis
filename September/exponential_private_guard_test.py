		...
		while values:
			...
			if to_as in ases_to_ips:
				...
				for to_ip in to_ips_in_as:
					raptor_weight = math.exp(0)
					if ips_to_bandwidth[to_ip] != 0:
						value = resilience*alpha + (1-alpha)*ips_to_bandwidth[to_ip]
						raptor_weight = math.exp((resilience*alpha + (1-alpha)*ips_to_bandwidth[to_ip])*epsilon)
					else:
						raptor_weight = math.exp(0)
						zeroes.add(to_ip)
